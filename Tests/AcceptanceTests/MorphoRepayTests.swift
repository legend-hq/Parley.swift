            ...
            test: .init(
                ...
                expect: .success(
                    .multi([
                        .multicall(
                            [
                                .quotePay(payment: .amt(0.00223, .usdc), payee: .stax, quote: .basic),
                                .bridge(
                                    bridge: "Across",
                                    srcNetwork: .worldChain,
                                    destinationNetwork: .base,
                                    // Solver chooses max bridge; ensure call matches actual routing
                                    inputTokenAmount: .amt(6.999954, .usdc),
                                    outputTokenAmount: .amt(6.995532, .usdc),
                                    cappedMax: true
                                ),
                            ],
                            executionType: .immediate
