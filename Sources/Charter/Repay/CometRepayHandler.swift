...
// MARK: - CometRepayHandler

struct CometRepayHandler: RepayIntentHandler {
    typealias RepayIntent = Charter.CometRepayIntent
    ...
    func createWithdrawCollateralRoute(
        ...
        .cometWithdrawCollateral(isMax: isMax)
    }
    ...
    func queryExistingDebt(
        ...
        let borrowKey = Folio.BalanceType.borrowMarket(
            borrowMarket: .comet(
                network: network,
                comet: intent.comet,
                underlyingSymbol: intent.assetSymbol
            ),
            wallet: repayer
        )
    ...
    func queryExistingCollateral(
        ...
        // Verify the comet market exists in Atlas
        guard Atlas.getCometMarket(network: network, comet: intent.comet) != nil
        else {
            return resources
        }
        ...
        for (balanceType, balance) in folio.balances {
            ...
            if !requestedCollateralSymbol.isEmpty && requestedCollateralSymbol != tokenSymbol {
                continue
            }
    ...
    // No override needed - the default implementation in RepayIntentHandler now follows
    // the BorrowHandler pattern: generate routes, then transform them to combined operations.
    // This maintains single commodity flow while avoiding the crashes.
}
