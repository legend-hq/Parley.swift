...
charter="${git_root}/Mercator.swift/Sources/Charter/Charter.swift"
...
if grep -q "public static let version = \"${target_version}\"" "${charter}"; then
  printf "✓ Updated Mercator.swift Charter version to %s.\n" "${target_version}"
  rm ${charter}.bak
else
  >&2 printf "✗ Failed to update Mercator.swift Charter version to %s.\n" "${target_version}"
